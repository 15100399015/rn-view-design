...
class ResolveCommand {
  ...
  constructor(config, callback = () => {}) {
    ...
    const hitCommand = config.commandls.find(
      (item) => item.command === findCommand
    );
    if (!hitCommand) exitProcess(findCommand + " Command not defined");

    const hitOptions = findOptions.filter((option) =>
      hitCommand.options.includes(option)
    );
...
class Logger {
  ...
  static changeStyle = (input, style) =>
    `\x1b[${this.generatorStyle(style)}m${input}\x1b[0m`;
  static bColorMap = {
    black: "40",
    red: "41",
    green: "42",
  ...
  static red(text, f = false) {
    return this.changeStyle(
      text,
      f ? [this.fColorMap.red] : [this.fColorMap.white, this.bColorMap.red]
    );
  }
  static green(text, f = false) {
    return this.changeStyle(
      text,
      f ? [this.fColorMap.green] : [this.fColorMap.white, this.bColorMap.green]
    );
  }
  static yellow(text, f = false) {
    return this.changeStyle(
      text,
      f
        ? [this.fColorMap.yellow]
        : [this.fColorMap.white, this.bColorMap.yellow]
    );
  }
  static blue(text, f = false) {
    return this.changeStyle(
      text,
      f ? [this.fColorMap.blue] : [this.fColorMap.white, this.bColorMap.blue]
    );
  }
